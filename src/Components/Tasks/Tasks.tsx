...
const Tasks = () => {
  ...
  const [edit, setedit] = useState<ShopifyTaskInterface>({
    isCustomSizes: false,
    __taskNumber: 1,
    sizes: {},
    isRun: false,
  ...
  const handleStartAll = () => {
    dispatch(
      TaskDispatcher('EDIT_ALL_CHECKOUTS_STATE', {
        taskId: '',
        message: { level: 'LOW', state: 'started' },
      })
    );
    // dispatch({
    //   type: EDIT_ALL_CHECKOUTS_STATE,
    //   payload: { message: { level: 'LOW', state: 'started' } },
    // });
    if (
      !Object.keys(tasks).filter(
        (taskId) =>
          tasks[taskId].isRun && (tasks[taskId].shop == 'shopify') == true
      ).length
    ) {
      for (let url of Object.keys(SITES)) new ShopifyMonitor(url).Parse();
    }
    dispatch(
      TaskDispatcher('RUN_STOP_ALL_TASKS', { isRun: true, taskId: '' })

      /* { type: RUN_STOP_ALL_TASKS, payload: { isRun: true } } */
    );
  };
  const handleStopAll = () => {
    dispatch(
      TaskDispatcher('EDIT_ALL_CHECKOUTS_STATE', {
        taskId: '',
        message: { level: 'ERROR', state: 'stopped' },
      })
      /* {
      type: EDIT_ALL_CHECKOUTS_STATE,
      payload: { message: { level: 'ERROR', state: 'stopped' } },
    } */
    );
    dispatch(
      TaskDispatcher('RUN_STOP_ALL_TASKS', { isRun: false, taskId: '' })
      /* { type: RUN_STOP_ALL_TASKS, payload: { isRun: false } } */
    );
  };
  const handleDeleteAll = () => {
    ...
    dispatch(
      TaskDispatcher('REMOVE_ALL_TASKS', { taskId: '' })
      /* { type: REMOVE_ALL_TASKS, payload: {} } */
    );
  };
  return (
    <div
        ...
        <div className="container">
          <div className="task_buttons_container">
            <button
              ...
              className="net_button_success tasks_buttons"
              id="startAllTasks"
            >
              Run
            </button>
            <button
              ...
              className="net_button_danger tasks_buttons"
              id="stopAllTasks"
            >
              Stop
            </button>
            <button
              ...
              className="net_button_secondary tasks_buttons"
              id="throwoffAllTasks"
            >
              Reset
            </button>
        ...
        <div className="modal-dialog modal-lg modal-dialog-centered">
          <div className="modal-content">
            ...
            <div className="modal-body">
              <form
                onSubmit={(event) => saveEdited(event)}
              ...
              >
                ...
                <div className="row g-6">
                  ...
                  <div className="col">
                    ...
                    >
                      ...
                      {Object.keys(profiles).map((profile: string, index) => (
                  ...
                  <div className="col">
                    ...
                    <select
                      /* @ts-ignore */
                      onChange={changeHandler}
                      value={edit.proxyProfile}
                      className="form-select"
                      id="proxyProfile"
                      required
                    >
                      <option value="">Выбрать...</option>
                      {Object.keys(proxyProfiles).map(
                ...
                <br />
                <button className="btn btn-primary" id="editShopifyTaskButton">
                  Сохранить изменения
                </button>
                <br />
              </form>
