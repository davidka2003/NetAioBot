...
export const tasksReducer = (state = defaultState ,action:ActionType)=>{
    ...
    switch (action.type){
        ...
        case RUN_STOP_TASK:
            currentState = {...state}
            Object.keys(SITES).forEach(site=>!currentState[action.payload.taskId].checkoutsBypass[site]?currentState[action.payload.taskId].checkoutsBypass[site]={}:null)
            ...
            /*  */
            Object.keys(currentState[action.payload.taskId].checkoutsBypass).map(site=>Object.keys(currentState[action.payload.taskId].checkoutsBypass[site]).map(bypass=>console.log(currentState[action.payload.taskId].checkoutsBypass[site][bypass].bypass.setStop=!action.payload.isRun)))
/*  */
