...
// request.debug = true
const getProxy = (proxyProfile: string): string[] =>
  store.getState().proxy[proxyProfile].proxy;
const changeProxy = () => {
...
const getTasks = (): { [key: string]: ShopifyTaskInterface } => {
  ...
  let shopifyTasks = {};
  Object.keys(tasks)
    .filter((task) => tasks[task].shop == 'shopify')
    .forEach(
      (taskId) => (shopifyTasks = { ...shopifyTasks, [taskId]: tasks[taskId] })
    );
  return shopifyTasks;
};
// const getSettings = ()=>store.getState().settings
const getProfile = (profile: string): ProfileInterface =>
  store.getState().profiles[profile];
const editCheckoutState = (
  taskId: string,
  state: { level: 'LOW' | 'ERROR' | 'SUCCESS'; state: string }
) =>
  store.dispatch({
    type: EDIT_CHECKOUT_STATE,
    payload: { taskId, message: state },
  });
...
class BypassQueueLink {
  ...
  constructor(
    private url: string,
    public products: any,
    private proxyProfile: string = 'noProxy'
  ) {
    this.AddToCart();
  }
  getRandomSize() {
    let sizes = this.products.products
      .map((item: any) =>
        Object.keys(item.variants).filter(
          (size) => item.variants[size].available
        )
      )
      .flat();
  ...
  async AddToCart() {
    ...
    await request
      ...
      .then((r: any) => r.statusCode)
      .catch(console.log);
    if (this.stop) return; //**dispatch error state */
    if (
      !(
        await request
          .get(`${this.url}/cart.json`, {
  ...
  async GoToCheckout() {
    ...
    await request
      ...
      .then(async (resp: any) => {
        if (
          ['checkpoint', 'queue', 'cart', 'throttle'].some((string) =>
            resp.request.href.includes(string)
          )
        ) {
...
class Checkout {
  ...
  constructor(
    ...
    for (let bypass in bypasses) {
      if (
        !bypasses[bypass].used &&
        bypasses[bypass].bypass.getBypass.checkoutUrl.length
      ) {
        this.checkoutUrl = bypasses[bypass].bypass.getBypass.checkoutUrl;
        this.cookieJar = bypasses[bypass].bypass.getBypass.cookieJar;
        store.dispatch({
          type: USE_CHECKOUT_BYPASS,
          payload: { checkoutBypassId: bypass, taskId },
        });
        break;
      }
    }
  ...
  async AddToCart() {
    ...
    await request
      ...
      .catch((e: any) => {
        if (e.statusCode == 422) {
          ...
          /* Dispatch this error */
          editCheckoutState(this.taskId, {
            level: 'LOW',
            state: JSON.parse(e.error).description,
          });
        } else {
          console.log(JSON.parse(e.error).description);
          /* Dispatch this error */
    ...
    this.totalPrice = request
      ...
      .then(
        (r: { items_subtotal_price?: string | number }) =>
          r.items_subtotal_price
      )
      .catch(console.log);
        ...
        switch (true) {
            ...
            if (Object.keys(captchaHeader).length) {
              ...
              await request
                ...
                .then((r: any) => {
                  ...
                  this.checkoutUrl = r.request.href;
                })
          ...
          case this.checkoutUrl.includes('throttle'):
            ...
            await request
              ...
              .then(async (response: any) => {
                this.checkoutUrl = await response.request.href;
                if (this.checkoutUrl.includes('stock_problems'))
                  return await this.AddToCart();
                this.authenticity_token = $(
                  'input[name="authenticity_token"][type="hidden"]'
                )?.attr('value');
  ...
  async ShippingConfirm(): Promise<any> {
    ...
    await request
      ...
      .catch(console.log);
    if (this.stop) return; //**dispatch error state */
    let ship = await request
      .get(
        `${
          this.url
        }/cart/shipping_rates.json?shipping_address[zip]=${profile?.postCode?.replace(
          ' ',
          '+'
        )}&shipping_address[country]=${profile?.country?.replace(
          ' ',
          '+'
    ...
    this.totalPrice =
      parseInt((await this.totalPrice).toString()) +
      parseInt((100 * parseFloat(ship_prc)).toString());
    this.shippingOption = encodeURIComponent(
      (ship_src + '-' + ship_opt + '-' + ship_prc).replace(/ /g, '%20')
    )
      .replace('(', '%28')
      .replace(')', '%29');
  ...
  async ConfirmPayment() {
    ...
    await request
      .post(this.checkoutUrl, {
        ...
        proxy: Change(this.url, this.proxyProfile),
        body: `_method=patch&authenticity_token=${
          this.authenticity_token
        }&previous_step=payment_method&step=&s=${paymentToken}&checkout%5Bpayment_gateway%5D=${
          this.checkoutGateway
        }&checkout%5Bcredit_card%5D%5Bvault%5D=false&checkout%5Bdifferent_billing_address%5D=false&checkout%5Btotal_price%5D=${await this
...
export class ShopifyMonitor {
  constructor(public url = 'https://kith.com') {}
  Parse = async () => {
    console.log(1);

    try {
      let response = await request
        .get(this.url + '/products.json?limit=99999', {
          json: true,
          proxy: Change(this.url, 'noProxy'),
        })
        .catch(() => null);
      ...
      if (
        !Object.keys(tasks)
          .map((task) => tasks[task].isRun)
          .filter((isRun) => isRun).length
      )
        return;
      for (let item in products) {
        let variants = products[item]['variants'];
        let Include = (name: string) =>
          products[item]['title'].toLocaleLowerCase().includes(name);
        ...
        for (let task in tasks) {
          ...
          if (
            tasks[task].isRun &&
            Object.keys(tasks[task].checkoutsBypass?.[this.url] || {}).length <
              tasks[task].checkoutsAmount
          ) {
            /* generate bypass */
            store.dispatch({
              type: ADD_CHECKOUT_BYPASS,
              payload: {
                url: this.url /*  */,
                checkoutBypassId: id(),
                taskId: tasks[task].taskId,
                checkoutBypass: new BypassQueueLink(
                  this.url,
                  response,
              ...
              case RELEASE:
                for (let size in variants) {
                  if (
                    ...
                    Object.keys(tasks[task].checkouts || {}).length <
                      tasks[task].checkoutsAmount
                  ) {
